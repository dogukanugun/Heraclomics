...
# Doublet Removal Module UI Function
doubletRemovalUI <- function(id) {
  ns <- NS(id)
  tagList(
    ...
    fluidRow(
      box(
        ...
        solidHeader = TRUE,
        collapsible = TRUE,
        actionButton(ns("continue_to_load_second_data"), "Continue to Next Step", icon = icon("arrow-right"))
      )
    )
  )
}

doubletRemovalServer <- function(id, rv) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    # Reactive values to store processed data
    processed_data <- reactiveVal(NULL)
    proceed_to_load_second_data <- reactiveVal(FALSE)
    
    # Disable certain buttons initially
    observe({
      shinyjs::disable(ns("simulate_doublets"))
      shinyjs::disable(ns("remove_doublets"))
      shinyjs::disable(ns("continue_to_load_second_data"))
    })
    ...
    observeEvent(input$start_doublet_removal, {
      ...
      showModal(modalDialog("Doublet Removal in progress. Please wait...", footer = NULL))
      
      withProgress(message = 'Preprocessing Data and Detecting Doublets...', value = 0, {
        data <- rv$seurat_object
        ...
        rv$seurat_object <- data
        processed_data(data)
      })
      
      removeModal()
      shinyalert("Success", "Starting Doublet Removal completed successfully! Now you can Simulate Doublets", type = "success")
    ...
    observeEvent(input$skip_doublet_removal, {
      req(rv$seurat_object)
      processed_data(rv$seurat_object)
      
      shinyalert("Skipped", "Doublet removal has been skipped.", type = "info")
      
      # Enable Next Step button
      shinyjs::enable(ns("continue_to_load_second_data"))
    })
    ...
    observeEvent(input$simulate_doublets, {
      ...
      showModal(modalDialog("Simulating Doublets. Please wait...", footer = NULL))
      ...
      removeModal()
      shinyalert("Success", "Doublets have been simulated and detected successfully!", type = "success")
      ...
      output$plot_output <- renderPlotly({
        ...
        data <- processed_data()
        plot_type <- input$plot_type  # Get selected plot type
    ...
    observeEvent(input$remove_doublets, {
      ...
      rv$seurat_object <- singlet_data
      processed_data(singlet_data)
      
      shinyalert("Success", "Doublets have been removed successfully!", type = "success")
    ...
    observeEvent(input$continue_to_load_second_data, {
      shinyalert(
        title = "Proceed to Next Step?",
        ...
        callbackR = function(value) {
          if (isTRUE(value)) {
            # Debugging print
            print("Confirmed: Proceeding to Load Second Dataset.")
            # Set reactive value to TRUE
            proceed_to_load_second_data(TRUE)
          }
        }
      )
    })
    
    return(list(proceed_to_load_second_data = proceed_to_load_second_data))
  })
}
